...
class unetConv2(nn.Module):
    ...
    Parameters:
        ...
        act         -- flag to indicate activation between linear layers in SE 
                        (relu vs. prelu)
    ''' 
    def __init__(self, in_size, out_size, is_batchnorm, use_se = False, use_prelu = False, max_input_h=64, max_input_w=64, boxdown=True, boxcres=None):
       ...
       # nn.Conv2d(dim_in, dim_ou//n_boxes, 1, 1, 1),
        if is_batchnorm:
            if boxdown==True:
                n_boxes = 4
                ...
                dim_in = in_size
                dim_ou = out_size
                self.conv1 = nn.Sequential(nn.Conv2d(dim_in, dim_ou//n_boxes, kernel_size = 1, stride = 1,padding = 0),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor), nn.BatchNorm2d(dim_ou), nn.ReLU()
                )
            ...
            if boxdown==True:   
                self.conv2 = nn.Sequential(nn.Conv2d(dim_ou, dim_ou//n_boxes, kernel_size = 1, stride = 1,padding = 0),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor), nn.BatchNorm2d(dim_ou), nn.ReLU()
                )
                self.h, self.w = self.h // 2, self.w // 2
            else:
...
class unetUp(nn.Module):
    ...
    '''
    def __init__(self, in_size, out_size, is_deconv,max_input_h=64, max_input_w=64, boxup=True):
        ...
        if is_deconv:
            ...
            if boxup==True:
                ...
                self.up = nn.Sequential(nn.ConvTranspose2d(in_size, out_size//n_boxes, kernel_size=2, stride=2),BoxConv2d(out_size//n_boxes, n_boxes, self.h, self.w, reparametrization_factor=reparam_factor))
                self.h, self.w = self.h * 2, self.w * 2
            else:    
...
class unet(nn.Module):
    ...
    Parameters:
        ...
        is_batchnorm    -- boolean flag to indicate batch-normalization usage
    ''' 
    def __init__(self, in_channels=3, out_channels = 21, feature_scale=1, is_deconv=True, is_batchnorm=True,max_input_h=64, max_input_w=64, boxdown=False, boxcres = False, boxup = False):
        ...
        self.conv1 = unetConv2(self.in_channels, filters[0], self.is_batchnorm,self.boxdown)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)

        self.conv2 = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2)
        
        self.conv3 = unetConv2(filters[1], filters[2], self.is_batchnorm,self.boxdown)
        ...
        self.center = unetConv2(filters[3], filters[4], self.is_batchnorm,self.boxdown)


            
        # upsampling
        self.up_concat4 = unetUp(filters[4], filters[3], self.is_deconv,self.boxup)
        self.up_concat3 = unetUp(filters[3], filters[2], self.is_deconv)
        self.up_concat2 = unetUp(filters[2], filters[1], self.is_deconv,self.boxup)
