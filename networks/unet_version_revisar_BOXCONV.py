...
class unetConv2(nn.Module):
    ...
    Parameters:
        ...
        act         -- flag to indicate activation between linear layers in SE 
                        (relu vs. prelu)
    ''' 
    def __init__(self, in_size, out_size, is_batchnorm, use_se = False, use_prelu = False, max_input_h=64, max_input_w=64, boxdown=False, boxcres=False):
        ...
        if is_batchnorm:
            if boxdown==True:
                ...
                self.conv1 = nn.Sequential(
                    nn.Conv2d(dim_in, dim_ou//n_boxes, 1, 1, 1),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor), nn.BatchNorm2d(dim_ou//n_boxes), nn.ReLU()
                )
                self.h, self.w = self.h // 2, self.w // 2
                self.conv2 = nn.Sequential(
                    nn.Conv2d(dim_ou//n_boxes, dim_ou//n_boxes, 1, 1, 1),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor), nn.BatchNorm2d(dim_ou//n_boxes), nn.ReLU()
                )
                self.h, self.w = self.h // 2, self.w // 2
            else:
                self.conv1 = nn.Sequential(
            ...
            if boxdown==True:
                ...
                dim_ou = out_size
                self.conv1 = nn.Sequential(
                    nn.Conv2d(dim_in, dim_ou//n_boxes, 1, 1, 1),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor),nn.ReLU()
                
                )
                self.h, self.w = self.h // 2, self.w // 2
                self.conv2 = nn.Sequential(
                    nn.Conv2d(dim_ou//n_boxes, dim_ou//n_boxes, 1, 1, 1),BoxConv2d(dim_ou//n_boxes,n_boxes,self.h,self.w,reparametrization_factor=reparam_factor),nn.ReLU()
                )
                self.h, self.w = self.h // 2, self.w // 2
            else:
...
class unetUp(nn.Module):
    ...
    '''
    def __init__(self, in_size, out_size, is_deconv,max_input_h=64, max_input_w=64, boxup=False):
        ...
        if is_deconv:
            ...
            if boxup:
                ...
                self.up +=[nn.ConvTranspose2d(in_size, out_size//n_boxes, kernel_size=1, stride=2),BoxConv2d(out_size//n_boxes, n_boxes, self.h, self.w, reparametrization_factor=reparam_factor)]
                self.h, self.w = self.h * 2, self.w * 2
