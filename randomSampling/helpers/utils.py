...
class AeroCLoader(data.Dataset):
    ...
    def __init__(self, set_loc = 'left', set_type = 'train', size = 'small', hsi_sign = 'rad', 
                 hsi_mode = 'all', transforms = None, augs = None, tpercent=0, seedshuffle=4):
        ...
        if tpercent > 0:
            random.Random(seedshuffle).shuffle(self.filelist)
            if tpercent < 0.5: # Asumimos validacion
                self.filelist = self.filelist[:int(len(self.filelist)*tpercent)]
            else: # asumimos test
                self.filelist = self.filelist[int(len(self.filelist)*tpercent):]
