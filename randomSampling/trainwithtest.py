...
def checktest(epoch = 0):
    ...
    with torch.no_grad():
        for idx, (rgb_ip, hsi_ip, labels) in enumerate(testloader, 0):
    #        print(idx)
            N = hsi_ip.size(0)
    ...
    print('TEST: %d loss: %.3f' % (epoch + 1, valloss_fx / (idx+1)))
    testloss.append(valloss2.avg)
...
if __name__ == "__main__":
   ...
   ### 2. Network selections
    ...
    valset = AeroCLoader(set_loc = 'midright', set_type = 'train', size = 'small', \
                         hsi_sign=args.hsi_c, hsi_mode = hsi_mode, transforms = tx, tpercent = valsize)
    testset = AeroCLoader(set_loc = 'midright', set_type = 'train', size = 'small', \
                         hsi_sign=args.hsi_c, hsi_mode = hsi_mode, transforms = tx, tpercent = 1-valsize)
    ...
    for epoch in range(args.epochs):
        train(epoch)
        if epoch % 1 == 0:
            oa, mpca, mIOU, dice, IOU = checktest(epoch)
            print("---- TEST ----")
