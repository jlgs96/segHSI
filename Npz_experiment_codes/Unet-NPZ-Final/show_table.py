...
if __name__ == "__main__":
    ...
    for fs in listfs:
        for model in models:
            ...
            for seed in range(5):
                npzFile= np.load("/home/josel/software/segHSI/Npz_experiment_codes/Unet-NPZ-Final/NPZs/Validation/" + model + '_fs' + str(fs) + '_' + str(seed)+'_'+ 'VAL' + '.npz')
    ...
    for fs in listfs:
        for model in models:
            for seed in range(5):
                npzFile= np.load("/home/josel/software/segHSI/Npz_experiment_codes/Unet-NPZ-Final/NPZs/Test/"+ model + '_fs' + str(fs) + '_' + str(seed)+'_'+ 'TE' + '.npz')
                idfs    = listfs.index(fs)
                idmodel = models.index(model)
                #for idmet, met in enumerate(metrics):
